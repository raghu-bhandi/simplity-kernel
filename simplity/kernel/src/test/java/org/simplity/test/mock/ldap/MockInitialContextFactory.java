...
public class MockInitialContextFactory implements InitialContextFactory {
  ...
  public Context getInitialContext(Hashtable<?, ?> arg0) throws NamingException {
    if (context == null) {
      ...
      Mockito.when(context.lookup("vm://localhost?broker.persistent=false"))
          .thenReturn(connectionFactory);
      try {
        QueueConnection queueConnection = (QueueConnection) connectionFactory.createConnection();
        QueueSession queueSession =
            queueConnection.createQueueSession(false, javax.jms.Session.DUPS_OK_ACKNOWLEDGE);
        ...
        TopicSession topicSession =
            topicConnection.createTopicSession(false, javax.jms.Session.DUPS_OK_ACKNOWLEDGE);
